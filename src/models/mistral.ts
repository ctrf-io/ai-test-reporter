...
export async function mistral(systemPrompt: string, prompt: string, args: Arguments): Promise<string | null> {
    ...
    try {
        const response = await client.chat.completions.create({
            model: args.model || "mistral-medium",
            messages: [
...
export async function mistralFailedTestSummary(report: CtrfReport, args: Arguments, file?: string, log = false): Promise<CtrfReport> {
    ...
    for (const test of failedTests) {
        ...
        const response = await mistral(systemPrompt, prompt, args);
